...
Adds Certificate Verification fields (QR code) to the Event table.
...
MIGRATIONS = [
    {
        'name': 'verify_enabled',
        'sql': 'ALTER TABLE events ADD COLUMN verify_enabled BOOLEAN DEFAULT 0',
        'check': "SELECT name FROM pragma_table_info('events') WHERE name='verify_enabled'"
    },
    {
        'name': 'qr_position_x',
        'sql': 'ALTER TABLE events ADD COLUMN qr_position_x FLOAT DEFAULT 85.0',
        'check': "SELECT name FROM pragma_table_info('events') WHERE name='qr_position_x'"
    },
    {
        'name': 'qr_position_y',
        'sql': 'ALTER TABLE events ADD COLUMN qr_position_y FLOAT DEFAULT 85.0',
        'check': "SELECT name FROM pragma_table_info('events') WHERE name='qr_position_y'"
    },
    {
        'name': 'qr_size',
        'sql': 'ALTER TABLE events ADD COLUMN qr_size FLOAT DEFAULT 12.0',
        'check': "SELECT name FROM pragma_table_info('events') WHERE name='qr_size'"
    },
]
...
if __name__ == '__main__':
    print("=" * 50)
    print("Certificate Portal - Database Migration v3")
    print("Certificate Verification (QR code)")
    print("=" * 50)
    print()
